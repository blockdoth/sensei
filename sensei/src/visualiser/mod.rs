...
impl Visualiser {
    ...
    fn calculate_dynamic_bounds(data_points: &[(f64, f64)]) -> [f64; 2] {
        ...
        if min_val.is_infinite() || max_val.is_infinite() {
            return [0.0, 1.0]; // Empty data: return convention, skip padding
        }
        
        // Check for the case where all y-values are the same
        if (max_val - min_val).abs() < f64::EPSILON {
            // All y-values are the same: apply specific adjustment and return, skipping general padding.
            return [min_val - 0.5, max_val + 0.5];
        }

        // Case: Normal data with a range of y-values. Apply general padding.
        // No need for adjusted_min_val/adjusted_max_val here as the previous cases return early.
...
fn generate_chart_from_data(data: Vec<(f64, f64)>, title_str: &str) -> CharmingChart {
    ...
    let x_data: Vec<String> = data.iter().map(|(x, _)| x.to_string()).collect();
