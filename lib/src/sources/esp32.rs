...
pub struct Esp32Source {
    ...
    ack_waiters: Arc<Mutex<HashMap<u8, Sender<Result<Vec<u8>, ControllerError>>>>>,
}
...
impl Esp32Source {
    ...
    pub async fn send_esp32_command(
        ...
        if !self.is_running.load(AtomicOrdering::SeqCst) {
            return Err(ControllerError::Controller(
        ...
        {
            let mut waiters = self.ack_waiters.lock().map_err(|_| ControllerError::Controller("ACK waiter lock poisoned".to_string()))?;
            if waiters.insert(cmd as u8, ack_tx_local).is_some() {
                warn!("Overwriting ACK waiter for command {:?}. This might indicate a logic issue or a previous command timed out without cleanup.", cmd);
            }
        }
        ...
        tokio::task::spawn_blocking(move || {
            let mut port_guard = port_clone.lock().map_err(|_| std::io::Error::new(std::io::ErrorKind::Other, "Port lock poisoned"))?;
        ...
        }).await.map_err(|e| ControllerError::Controller(format!("Task for serial write panicked: {}", e)))??;
        ...
        let ack_result = tokio::task::spawn_blocking(move || {
            ack_rx_local.recv_timeout(ack_timeout)
        }).await.map_err(|e| ControllerError::Controller(format!("Task for ACK receive panicked: {}", e)))?;

        // Ensure waiter is removed
        self.ack_waiters.lock().map_err(|_| ControllerError::Controller("ACK waiter lock poisoned during cleanup".to_string()))?.remove(&(cmd as u8));

        match ack_result {
            Ok(ack_payload_result) => { // ack_payload_result is Result<Vec<u8>, ControllerError>
                match ack_payload_result {
                    Ok(ack_data) => {
                        info!("ACK received for command: {:?}", cmd);
                        Ok(ack_data)
                    }
                    Err(e) => { // Error explicitly sent from reader (e.g., ESP32 reported error)
            ...
            Err(RecvTimeoutError::Timeout) => {
                ...
                Err(ControllerError::Controller(format!("ACK timeout for ESP32 command {:?}", cmd)))
            }
            Err(RecvTimeoutError::Disconnected) => {
                error!("ACK channel disconnected for command: {:?}. Reader thread might have died.", cmd);
                Err(ControllerError::Controller(
        ...
        while is_running.load(AtomicOrdering::Relaxed) {
            let bytes_read_from_port = {
                ...
                if let Some(port) = port_guard.as_mut() {
                    ...
                    if !is_running.load(AtomicOrdering::Relaxed) { break; } // Exit if stopping
            ...
            match bytes_read_from_port {
                ...
                Err(_) => { // Serial read error, break the loop.
            ...
            loop {
                ...
                if let Some(pos) = partial_buffer.windows(ESP_PACKET_PREAMBLE_ESP_TO_HOST.len()).position(|w| w == ESP_PACKET_PREAMBLE_ESP_TO_HOST) {
                    if pos > 0 { partial_buffer.drain(0..pos); }
                    if partial_buffer.len() < ESP_TO_HOST_MIN_HEADER_SIZE { break; }
                    ...
                    if partial_buffer.len() < total_packet_len_on_wire { break; }
                    ...
                    if payload_len_signed < 0 { // ACK
                        if actual_payload_content.is_empty() {
                            warn!("ACK packet with empty payload."); continue;
                        }
                        ...
                        let mut waiters_guard = match ack_waiters.lock() { Ok(g) => g, Err(_) => {error!("ACK Waiter lock poisoned in reader"); continue;}};
                        if let Some(ack_tx_specific) = waiters_guard.remove(&cmd_byte) { // Remove to consume the waiter
                            if let Err(e) = ack_tx_specific.try_send(Ok(ack_data)) {
                                warn!("Failed to send ACK for cmd 0x{:02X} to specific waiter: {}", cmd_byte, e.to_string());
                            }
                        } else {
             ...
             // Yield for other tasks if buffer processing was fast and loop is tight
            if new_bytes.is_empty() && partial_buffer.len() < ESP_TO_HOST_MIN_HEADER_SIZE {
...
impl DataSourceT for Esp32Source {
    async fn start(&mut self) -> Result<(), DataSourceError> {
        ...
        let mut port = match port_result {
            ...
            Err(e) => {
                 ...
                 return Err(DataSourceError::Io(std::io::Error::new(e.kind(), e.to_string())));
            }
        };
        
        port.clear(ClearBuffer::All).map_err(|e| {
            DataSourceError::Io(std::io::Error::new(e.kind(), format!("Failed to clear serial buffers: {}", e)))
        })?;
    ...
    async fn stop(&mut self) -> Result<(), DataSourceError> {
        ...
        if let Some(handle) = self.reader_handle.take() {
            ...
            if let Err(e) = handle.join() { // This might block for a bit if thread is stuck
    ...
    async fn read(&mut self, buf: &mut [u8]) -> Result<usize, DataSourceError> {
        ...
        let result_opt_payload = tokio::task::spawn_blocking(move || {
             match csi_data_rx_clone.recv_timeout(Duration::from_millis(1)) { // Very short timeout for non-blocking feel
        ...
        }).await.map_err(|e| DataSourceError::Controller(format!("Read task panicked: {}", e)))?; // Error from spawn_blocking join
