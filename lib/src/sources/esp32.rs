...
use std::sync::{
    Arc, Mutex,
    atomic::{AtomicBool, Ordering as AtomicOrdering}, PoisonError,
};
...
const ESP_TO_HOST_MIN_HEADER_SIZE: usize = ESP_PACKET_PREAMBLE_ESP_TO_HOST.len() + 2; // Preamble + Type/Length (2 bytes)

const DEFAULT_ACK_TIMEOUT_MS: u64 = 2000;
const DEFAULT_CSI_BUFFER_SIZE: usize = 100;
const SERIAL_READ_TIMEOUT_MS: u64 = 100;
const SERIAL_READ_BUFFER_SIZE: usize = 4096;
...
// Helper for poisoned mutexes in Esp32Source methods
fn handle_poisoned_ack_waiters<T>(err: PoisonError<T>) -> ControllerError {
    ControllerError::Execution(format!("ACK waiter lock poisoned: {err}"))
}
fn handle_poisoned_port_lock<T>(err: PoisonError<T>) -> std::io::Error {
    error!("Serial port lock poisoned: {err}");
    std::io::Error::new(std::io::ErrorKind::Other, format!("Port lock poisoned: {err}"))
}
...
impl Esp32Source {
    ...
    pub async fn send_esp32_command(
        ...
        if !self.is_running.load(AtomicOrdering::SeqCst) {
            return Err(ControllerError::Execution("ESP32 source is not running. Cannot send command.".to_string()));
        }
        ...
        if let Some(d) = data {
            ...
            if d.len() > max_data_len {
                return Err(ControllerError::InvalidParams(format!("Command data too large: {} bytes (max {})", d.len(), max_data_len)));
            }
            command_packet[data_offset..data_offset + d.len()].copy_from_slice(&d);
        }

        let (ack_tx_local, ack_rx_local) = bounded(1);
        {
            let mut waiters = self.ack_waiters.lock().map_err(handle_poisoned_ack_waiters)?;
            if waiters.insert(cmd as u8, ack_tx_local).is_some() {
                warn!("Overwriting ACK waiter for command {cmd:?}. This might indicate a logic issue or a previous command timed out without cleanup.");
            }
        }

        let port_clone = Arc::clone(&self.port);
        tokio::task::spawn_blocking(move || {
            let mut port_guard = port_clone.lock().map_err(handle_poisoned_port_lock)?;
            if let Some(port_ref) = port_guard.as_mut() {
                ...
                Err(std::io::Error::new(std::io::ErrorKind::NotConnected, "Serial port not open for sending command"))
            }
        }).await.map_err(|e| ControllerError::Execution(format!("Task for serial write panicked: {e}")))??;
        ...
        let ack_result = tokio::task::spawn_blocking(move || ack_rx_local.recv_timeout(ack_timeout))
            .await.map_err(|e| ControllerError::Execution(format!("Task for ACK receive panicked: {e}")))?;
        
        self.ack_waiters.lock().map_err(handle_poisoned_ack_waiters)?.remove(&(cmd as u8));

        match ack_result {
            Ok(ack_payload_result) => match ack_payload_result {
                Ok(ack_data) => {
            ...
            Err(RecvTimeoutError::Disconnected) => {
                ...
                Err(ControllerError::Execution("ESP32 ACK channel disconnected; reader thread likely terminated.".to_string()))
            }
        }
    }
        ...
        while is_running.load(AtomicOrdering::Relaxed) {
            let bytes_read_from_port = {
                let mut port_guard = match port_arc.lock() {
                    Ok(guard) => guard,
                    Err(p_err) => {
                        error!("Reader task: Port mutex poisoned: {p_err}. Terminating.");
                        break;
                    }
                };
                if let Some(port) = port_guard.as_mut() {
                    ...
                    if !is_running.load(AtomicOrdering::Relaxed) { break; }
            ...
            match bytes_read_from_port {
                ...
                Err(_) => { // Serial error caused termination
                    is_running.store(false, AtomicOrdering::Relaxed); // Ensure flag is set
                    break;
                }
            }

            loop { // Inner loop for processing partial_buffer
                if partial_buffer.len() < ESP_TO_HOST_MIN_HEADER_SIZE { break; }

                if let Some(pos) = partial_buffer.windows(ESP_PACKET_PREAMBLE_ESP_TO_HOST.len()).position(|w| w == ESP_PACKET_PREAMBLE_ESP_TO_HOST) {
                    if pos > 0 {
                        debug!("Preamble found at pos {pos}. Discarding {pos} bytes before preamble.");
                        partial_buffer.drain(0..pos);
                    }
                    if partial_buffer.len() < ESP_TO_HOST_MIN_HEADER_SIZE { break; } // Check again after drain
                    ...
                    if total_packet_len_on_wire < ESP_TO_HOST_MIN_HEADER_SIZE {
                        error!("Declared total packet length {total_packet_len_on_wire} is less than minimum header size {ESP_TO_HOST_MIN_HEADER_SIZE}. Corrupted packet. Discarding.");
                    ...
                    if is_ack_packet {
                        // *** Applied Fix for ACK handling ***
                        if actual_payload_content.is_empty() {
                            warn!("ACK packet received but its payload (expected to contain cmd_byte) is empty. Length field: {esp_length_field_val_signed}, Total packet len on wire: {total_packet_len_on_wire}. Skipping.");
                            continue; 
                        }
                        let cmd_byte = actual_payload_content[0];
                        let ack_data = if actual_payload_content.len() > 1 { actual_payload_content[1..].to_vec() } else { Vec::new() };
                        debug!("Received ACK for command byte 0x{cmd_byte:02X} with data len {}", ack_data.len());
                        
                        let mut waiters_guard = match ack_waiters.lock() {
                            Ok(g) => g,
                            Err(p_err) => {
                                error!("ACK Waiter lock poisoned in reader: {p_err}");
                                continue; // Or break, depending on desired error handling for poisoned lock here
                            }
                        };
                        ...
                        if csi_data_tx.is_full() {
                            ...
                            warn!("Failed to send CSI data to channel: {e}");
                             if e.is_disconnected() { // If disconnected, the main loop will stop.
                    ...
                    break; // Break inner loop, wait for more data from port
                }
            } // End inner loop

            if !new_data_was_read_in_this_iteration && partial_buffer.len() < ESP_TO_HOST_MIN_HEADER_SIZE {
                if !is_running.load(AtomicOrdering::Relaxed) { break; }
                thread::sleep(Duration::from_millis(10)); // Small pause before trying to read port again
            }
        } // End while is_running

        is_running.store(false, AtomicOrdering::Relaxed); // Ensure this is set on exit
        if let Ok(mut waiters) = ack_waiters.lock() {
            waiters.clear(); // Clear any remaining waiters
        } else {
            error!("ACK Waiters lock poisoned during reader task cleanup.");
        }
...
impl DataSourceT for Esp32Source {
    async fn start(&mut self) -> Result<(), DataSourceError> {
        ...
        info!("Starting ESP32Source on port {} at {} baud.", self.config.port_name, self.config.baud_rate);
        ...
        *self.port.lock().map_err(|p_err| DataSourceError::Controller(format!("Port lock poisoned on start: {p_err}")))? = Some(port);
    ...
    async fn stop(&mut self) -> Result<(), DataSourceError> {
        ...
        self.is_running.store(false, AtomicOrdering::SeqCst); // Signal reader thread

        if let Some(port_to_drop) = self.port.lock().map_err(|p_err| DataSourceError::Controller(format!("Port lock poisoned on stop: {p_err}")) )?.take() {
            drop(port_to_drop); // This should interrupt blocking reads in the reader thread
            debug!("Serial port explicitly dropped during stop.");
        }

        if let Some(handle) = self.reader_handle.take() {
            debug!("Waiting for ESP32 reader thread to join...");
            // Best effort join, with a timeout to prevent hanging indefinitely
            let jh_result = tokio::task::spawn_blocking(move || handle.join()).await;
            match jh_result {
                Ok(Ok(_)) => debug!("ESP32 reader thread joined."),
                Ok(Err(e)) => error!("ESP32 reader thread panicked or failed to join cleanly: {e:?}"),
                Err(je) => error!("Failed to spawn blocking task for joining reader thread: {je}"),
            }
        }
        
        self.ack_waiters.lock().map_err(|p_err| DataSourceError::Controller(format!("ACK Waiters lock poisoned on stop: {p_err}")) )?.clear();
    ...
    async fn read(&mut self, buf: &mut [u8]) -> Result<usize, DataSourceError> {
        if !self.is_running.load(AtomicOrdering::SeqCst) && self.csi_data_rx.is_empty() {
            ...
            return Ok(0); // Source stopped, no more data
        }

        let csi_data_rx_clone = self.csi_data_rx.clone();
        let is_running_clone = self.is_running.clone(); // Clone Arc<AtomicBool>

        let result_opt_payload = tokio::task::spawn_blocking(move || {
            // Poll is_running inside spawn_blocking before potentially blocking recv
            if !is_running_clone.load(AtomicOrdering::Relaxed) && csi_data_rx_clone.is_empty() {
                 return Err(DataSourceError::Controller("Source stopped and channel empty.".to_string()));
            }
            match csi_data_rx_clone.recv_timeout(Duration::from_millis(1)) {
                Ok(data_payload) => Ok(Some(data_payload)),
                Err(RecvTimeoutError::Timeout) => Ok(None), // No data currently available
                Err(RecvTimeoutError::Disconnected) => {
                    // If channel is disconnected, it implies reader thread is gone.
                    // Set is_running to false if it's not already.
                    is_running_clone.store(false, AtomicOrdering::Relaxed);
                    Err(DataSourceError::Controller("CSI data channel disconnected".to_string()))
                }
            }
        }).await.map_err(|e| DataSourceError::Controller(format!("Read task panicked: {e}")))?;

        match result_opt_payload {
            Ok(Some(data_payload)) => {
                if data_payload.len() > buf.len() {
                    warn!("User buffer (len {}) too small for ESP32 CSI data (len {}). Data truncated.", buf.len(), data_payload.len());
    ...
    fn drop(&mut self) {
        if self.is_running.load(AtomicOrdering::Relaxed) || self.reader_handle.is_some() {
            ...
            if let Ok(mut port_guard) = self.port.lock() {
                ...
                error!("Port lock poisoned during Esp32Source drop.");
            }
            if let Some(handle) = self.reader_handle.take() {
                 debug!("Reader thread handle taken in drop. Thread should self-terminate. Timed join attempt...");
                 // Non-blocking join or very short timeout can be attempted here, but generally risky in drop
                 // For this example, we'll rely on the thread self-terminating.
                 // If a join is absolutely necessary, it should be very short and handled carefully.
                 // For now, no join in drop to avoid potential deadlocks.
            }
        }
    }
}