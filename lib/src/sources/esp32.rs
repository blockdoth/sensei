...
impl Default for Esp32SourceConfig {
    fn default() -> Self {
        Self {
            port_name: "".to_string(),
            baud_rate: 3_000_000, // Common high speed for ESP32
...
impl Esp32Source {
    ...
    pub async fn send_esp32_command( // Changed to async fn
        &self,
        ...
        trace!("Prepared command packet for {:?}: {:02X?}", command, command_packet);
        ...
        let (ack_tx, ack_rx) = bounded(1);
        {
            ...
            if waiters_guard.insert(command as u8, ack_tx).is_some() {
                warn!(
                    "Overwriting ACK waiter for command {:?}. Previous command might have timed out.",
                    command
                );
            }
        }
        ...
        tokio::task::spawn_blocking(move || {
            let mut port_guard = port_clone.lock().map_err(|e: PoisonError<_>| {
                std::io::Error::new(
                    IoErrorKind::Other,
                    format!("Serial port lock poisoned for send: {e}"),
                )
            })?;

            if let Some(port) = port_guard.as_mut() {
                debug!("Sending command {:?} to ESP32 ({} bytes).", command_debug_clone, command_packet_clone.len());
        ...
        let ack_wait_result = tokio::task::spawn_blocking(move || {
            ack_rx.recv_timeout(ack_timeout)
        })
        .await; // Wait for spawn_blocking to complete
        ...
        let ack_payload_result = match ack_wait_result {
            ...
            Ok(Err(crossbeam_channel::RecvTimeoutError::Timeout)) => {
                ...
                Err(ControllerError::AckTimeout(format!(
                    "Timeout waiting for ACK for ESP32 command {:?}",
                    command
                )))
            }
            Ok(Err(crossbeam_channel::RecvTimeoutError::Disconnected)) => {
                error!(
                    "ACK channel disconnected for command: {:?}. Reader thread might have terminated.",
                    command
                );
            ...
            Err(join_err) => { // JoinError from spawn_blocking
                error!("Task for ACK receive panicked for command {:?}: {}", command, join_err);
                Err(ControllerError::Execution(format!("Task for ACK receive panicked: {join_err}")))
            }
        };
        
        // Ensure waiter is removed
        self.ack_waiters.lock().map_err(|e: PoisonError<_>| {
            ControllerError::Execution(format!("ACK waiter lock poisoned on cleanup: {e}"))
        })?.remove(&(command as u8));

        match ack_payload_result {
            Ok(Ok(ack_data)) => { // Inner Ok is from AckPayload = Result<Vec<u8>, ControllerError>
                info!("ACK received for command: {:?}", command);
                Ok(ack_data)
            }
            Ok(Err(controller_err)) => { // Inner Err is from AckPayload
                 error!("ESP32 reported error in ACK for command {:?}: {}", command, controller_err);
                 Err(controller_err)
            }
            Err(controller_err) => { // Error from recv_timeout handling (Timeout, Disconnected, Panic)
                Err(controller_err)
            }
        }
    }
    ...
    fn reader_task_loop(
        ...
        while is_running_arc.load(AtomicOrdering::Relaxed) {
            let bytes_read_this_iteration = {
                ...
                if let Some(port) = port_guard.as_mut() {
                    match port.read(&mut temp_serial_buf) {
                        Ok(0) => { // Should not happen with timeout, but handle defensively
                    ...
                    if !is_running_arc.load(AtomicOrdering::Relaxed) { break; } // Exit if shutting down
                    drop(port_guard); // Release lock before sleep
            ...
            if bytes_read_this_iteration > 0 {
                trace!("Reader thread: read {} bytes from serial. Buffer size: {}", bytes_read_this_iteration, current_read_buffer.len());
            }
            ...
            loop {
                    ...
                    .position(|window| window == ESP_PACKET_PREAMBLE_ESP_TO_HOST)
                {
                    if preamble_start_idx > 0 {
                        debug!(
                            "Discarding {} bytes before preamble.",
                            preamble_start_idx
                        );
                    ...
                    let mut len_bytes_cursor = std::io::Cursor::new(
                        &current_read_buffer[ESP_PACKET_PREAMBLE_ESP_TO_HOST.len()..ESP_TO_HOST_MIN_HEADER_SIZE],
                    );
                    let esp_length_field = match len_bytes_cursor.read_i16::<LittleEndian>() {
                        Ok(len) => len,
                        Err(e) => {
                            error!("Failed to read packet length field: {e}. Discarding minimal header to attempt recovery.");
                    ...
                    if total_packet_on_wire_len < ESP_TO_HOST_MIN_HEADER_SIZE {
                         error!("ESP32 reported invalid packet length: {total_packet_on_wire_len} (must be >= {ESP_TO_HOST_MIN_HEADER_SIZE}). Discarding preamble and length field.");
                         current_read_buffer.drain(0..ESP_TO_HOST_MIN_HEADER_SIZE);
                         continue;
                    }
                    ...
                    if current_read_buffer.len() < total_packet_on_wire_len {
                        trace!(
                            ...
                            current_read_buffer.len(), total_packet_on_wire_len
                        );
                        break; // Need more data for the full packet
                    }

                    // We have a full packet
                    let packet_with_header_and_len_field = current_read_buffer.drain(0..total_packet_on_wire_len).collect::<Vec<_>>();
                    let actual_payload_data = packet_with_header_and_len_field[ESP_TO_HOST_MIN_HEADER_SIZE..].to_vec();
                    
                    let is_ack_packet = esp_length_field < 0;

                    if is_ack_packet {
                        if actual_payload_data.is_empty() {
                            warn!("ACK packet received with empty payload (expected command byte). Skipping.");
                            continue;
                        }
                        ...
                        let mut waiters_guard = match ack_waiters_arc.lock() {
                             Ok(g) => g,
                             Err(p_err) => {
                                error!("ACK Waiter lock poisoned in reader: {p_err}. Cannot deliver ACK.");
                                continue;
                             }
                        };
                        if let Some(ack_sender_channel) = waiters_guard.remove(&cmd_byte_acked) {
                            if let Err(e) = ack_sender_channel.try_send(Ok(ack_data_payload)) {
                                warn!(
                                    "Failed to send ACK payload for cmd 0x{:02X} to waiting task: {:?}. Channel might be closed (timeout).",
                                    cmd_byte_acked, e
                                );
                            } else {
                                trace!("Successfully delivered ACK for 0x{:02X} to waiter.", cmd_byte_acked);
                            }
                        } else {
                            warn!(
                                "Received ACK for command 0x{:02X}, but no waiter was registered or it already timed out.",
                                cmd_byte_acked
                            );
                        }
                    } else { // CSI Data Packet
                        trace!("Received CSI Data packet (payload len {}). Forwarding.", actual_payload_data.len());
                        if csi_data_sender.is_full() {
                            warn!("CSI data channel is full. Discarding incoming ESP32 CSI packet. CLI might be blocked or slow.");
                    ...
                    if current_read_buffer.len() > SERIAL_READ_BUFFER_SIZE { // Heuristic
                         let discard_len = current_read_buffer.len() - (ESP_PACKET_PREAMBLE_ESP_TO_HOST.len().saturating_sub(1));
                         debug!("No preamble found in {} bytes of buffer. Discarding {} bytes to search again.", current_read_buffer.len(), discard_len);
            ...
            if bytes_read_this_iteration == 0 && current_read_buffer.len() < ESP_TO_HOST_MIN_HEADER_SIZE {
                if !is_running_arc.load(AtomicOrdering::Relaxed) { break; }
        ...
        is_running_arc.store(false, AtomicOrdering::Relaxed); // Ensure state is false
         if let Ok(mut waiters) = ack_waiters_arc.lock() {
            ...
            for (_, sender) in waiters.drain() {
                let _ = sender.try_send(Err(ControllerError::Execution("ESP32Source stopped; command aborted.".to_string())));
            }
        } else {
...
impl DataSourceT for Esp32Source {
    async fn start(&mut self) -> Result<(), DataSourceError> {
        if self.is_running.compare_exchange(false, true, AtomicOrdering::SeqCst, AtomicOrdering::Relaxed).is_err() {
        ...
        *self.serial_port.lock().map_err(|p_err| {
        ...
        let reader_handle = thread::Builder::new()
            .name(format!("esp32-reader-{}", self.config.port_name.replace("/","_")))
            .spawn(move || {
        ...
        info!("ESP32Source started successfully on {}.", self.config.port_name);
        Ok(())
    }

    async fn stop(&mut self) -> Result<(), DataSourceError> {
        if self.is_running.compare_exchange(true, false, AtomicOrdering::SeqCst, AtomicOrdering::Relaxed).is_err() {
            ...
            if let Some(handle) = self.reader_thread_handle.take() {
                debug!("ESP32Source was not marked as running, but reader thread handle exists. Attempting join.");
                 match handle.join() {
                    Ok(_) => debug!("Reader thread joined successfully after stop (was not marked running)."),
                    Err(e) => warn!("Error joining reader thread after stop (was not marked running): {:?}", e),
                }
            }
            return Ok(());
        }
        ...
        if let Ok(mut waiters) = self.ack_waiters.lock() {
            if !waiters.is_empty() {
                warn!("{} ACK waiters were still pending during stop. Notifying them of shutdown.", waiters.len());
                 for (_, sender) in waiters.drain() {
                    let _ = sender.try_send(Err(ControllerError::Execution("ESP32Source stopped; command aborted during shutdown.".to_string())));
                }
            }
        } else {
    ...
    async fn read(&mut self, buf: &mut [u8]) -> Result<usize, DataSourceError> {
        ...
        match self.csi_data_receiver.try_recv() {
            ...
            Err(TryRecvError::Empty) => {
                if !self.is_running.load(AtomicOrdering::Relaxed) && self.csi_data_receiver.is_empty(){
...
impl Drop for Esp32Source {
    fn drop(&mut self) {
        if self.is_running.load(AtomicOrdering::Relaxed) || self.reader_thread_handle.is_some() {
            info!("Dropping Esp32Source for {}: ensuring resources are cleaned up.", self.config.port_name);
            ...
            if let Ok(mut port_guard) = self.serial_port.lock() {
                ...
                error!("Serial port lock poisoned during Esp32Source::drop for {}.", self.config.port_name);
            }
            ...
            if let Some(handle) = self.reader_thread_handle.take() {
                debug!("Esp32Source::drop waiting for reader thread to join for {}.", self.config.port_name);
                if let Err(e) = handle.join() {
                    ...
                    debug!("Reader thread joined in Esp32Source::drop for {}.", self.config.port_name);
                }
            }
        }
    }
