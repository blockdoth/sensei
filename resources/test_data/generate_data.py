...
def generate_csi_row(max_cores, max_streams, max_subcarriers, always_generate_max=False):
    """
    ...
    num_cores = random.randint(1, max_cores) if not always_generate_max else max_cores                      # Random number of cores
    num_stream = random.randint(1, max_streams) if not always_generate_max else max_streams                 # Random number of antennas per core
    num_subcarriers = random.randint(1, max_subcarriers) if not always_generate_max else max_subcarriers    # Random number of subcarriers
    ...
    for _ in range(num_cores):
        for _ in range(num_stream):
            for _ in range(1):  # Assuming 1 stream per antenna
                csi.extend([complex(random.uniform(-1, 1), random.uniform(-1, 1)) for _ in range(num_subcarriers)])
...
def generate_csi_data_csv(file_path, num_rows=100, max_cores=2, max_streams=4, max_subcarriers=64, always_generate_max=False):
    """
    ...
    with open(file_path, "w", newline="") as csvfile:
        ...
        for _ in range(num_rows):
            row = generate_csi_row(max_cores, max_streams, max_subcarriers, always_generate_max)
            writer.writerow(row)
    # headers for the CSV file
    headers = [
        "timestamp",          # Timestamp (f64)
...
generate_csi_data_csv("csv/csi_data.csv", num_rows=100, max_cores=2, max_streams=2, max_subcarriers=2, always_generate_max=True)
